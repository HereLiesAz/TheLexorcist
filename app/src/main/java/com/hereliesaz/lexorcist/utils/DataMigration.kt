...
// import com.hereliesaz.lexorcist.model.Script // Not directly used here, using ScriptFromFile
// import com.hereliesaz.lexorcist.model.Template // Not directly used here, using TemplateFromFile
// import org.apache.poi.ss.usermodel.WorkbookFactory // Unused import
...
private data class ScriptFromFile(
    val id: String? = null,
...
private data class TemplateFromFile(
    val id: String? = null,
    val name: String? = null,
    val description: String? = null,
    val content: String? = null,
...
private data class ExtrasFromFile(
...
class DataMigration(private val context: Context) {
    ...
    fun migrate() {
        ...
        try {
            ...
            // allegationHeader.createCell(3).setCellValue("elements") // Not in current MasterAllegation definition
            ...
            // allegationHeader.createCell(7).setCellValue("relevantEvidence") // Not in current MasterAllegation definition
            ...
            allegations.forEachIndexed { index, allegation ->
                ...
                // row.createCell(3).setCellValue(gson.toJson(allegation.elements)) // Not in MasterAllegation
                ...
                // row.createCell(7).setCellValue(gson.toJson(allegation.relevantEvidence)) // Not in MasterAllegation
            }
        } catch (e: FileNotFoundException) {
            Log.e(TAG, "allegations_catalog.json not found. Skipping Allegations migration.", e)
        } catch (e: Exception) {
        ...
        try {
            ...
            exhibitHeader.createCell(1).setCellValue("type") // Corrected from "exhibit_type"
            ...
            Log.e(TAG, "exhibits_catalog.json not found. Skipping Exhibits migration.", e)
        } catch (e: Exception) {
        ...
        try {
            ...
            Log.e(TAG, "jurisdictions.json not found. Skipping Jurisdictions migration.", e)
        } catch (e: Exception) {
        ...
        try {
            ...
            val extrasType = object : TypeToken<ExtrasFromFile>() {}.type // Use ExtrasFromFile for type safety
            ...
            if (extras != null) {
                extras.scripts?.let {
                    ...
                    // id and court are in ScriptFromFile but not used in this sheet as per original code
            ...
            Log.e(TAG, "default_extras.json not found. Skipping DefaultExtras migration.", e)
        } catch (e: Exception) {
        ...
        try {
            ...
            Log.e(TAG, "statistical_weights.json not found. Skipping StatisticalWeights migration.", e)
        } catch (e: Exception) {
        ...
        try {
            ...
            Log.i(TAG, "Data migration to lexorcist_catalogs.xlsx successful.")
        } catch (e: Exception) {
